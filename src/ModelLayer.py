...
class ModelLayer:
	...
	def evaluate(self,sentence):
		...
		with torch.no_grad():
			...
			encoder_hidden_forward = self.encoder_model.initHidden()
			encoder_hidden_backward = self.encoder_model.initHidden()

			encoder_outputs_forward = torch.zeros(max_length, self.encoder_model.hidden_size, device=device)
			encoder_outputs_backward = torch.zeros(max_length, self.encoder_model.hidden_size, device=device)

			for ei in range(input_length):
			    encoder_output_forward, encoder_hidden_forward = self.encoder_model(input_tensor[ei],encoder_hidden_forward,True)
			    encoder_outputs_forward[ei] += encoder_output_forward[0, 0]

			for ei in range(input_length-1,-1,-1):
			    encoder_output_backward, encoder_hidden_backward = self.encoder_model(input_tensor[ei], encoder_hidden_backward,False)
			...
			for di in range(max_length):
			    ...
			    if forward_flag:
			        decoder_output_forward, decoder_hidden_forward, decoder_attention_forward = self.decoder_model(decoder_input_forward, decoder_hidden_forward, encoder_outputs_backward,True)
			    ...
			    if backward_flag:
			        decoder_output_backward, decoder_hidden_backward, decoder_attention_backward = self.decoder_model(decoder_input_backward, decoder_hidden_backward, encoder_outputs_forward,True)
			...
			if len(decoder_output_backward_list) < len(decoder_output_forward_list):
			    for i in range(len(decoder_output_backward_list)-1,len(decoder_output_forward_list)):
			        decoder_output.append(decoder_output_forward_list)
			    ...
			    for i in range(len(decoder_output_forward_list)-1,len(decoder_output_backward_list)):
			        decoder_output.append(decoder_output_backward_list)
	...
	def get_summary(self,input_text):
		...
		output_words = self.evaluate(input_text)
